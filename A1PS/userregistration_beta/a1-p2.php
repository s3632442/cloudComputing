...
<body>
	...
	<?php
        ...
        $query = "SELECT string_field_1 as COUNTRY_LABEL , service_label , status, value  FROM `A1_data.gsquarterly` as GS_Q JOIN `A1_data.country_classification` as COUNTRY_DATA on COUNTRY_DATA.string_field_0 = `country_code` JOIN `A1_data.services_classification` as S_C on GS_Q.code = S_C.code WHERE value IS NOT NULL AND string_field_1 != 'Total' AND product_type = 'Services' GROUP BY product_type , account, string_field_1, value, status, time_ref, service_label ORDER BY time_ref, value DESC LIMIT 30;";
		$queryJobConfig = $client->query($query);
		$queryResults = $client->runQuery($queryJobConfig);

		$str = "<h3> Top 30 services with the highest total trade surplus value </h3><div><table>".
		"<tr>" .
		"<th>Country</th>" .
		"<th>Product</th>" .
        "<th>Status</th>" .
